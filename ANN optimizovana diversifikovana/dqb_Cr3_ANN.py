...
Architecture
------------
Input (15) → Linear(64) → BN → ReLU → Dropout(0.311)
           → Linear(32) → BN → ReLU → Dropout(0.184)
           → Linear(1)

Pyramidal architecture (64→32) selected for optimal balance between
expressiveness and generalization on 207 training compounds.
Non-pyramidal (64→64) showed overfitting in prospective validation.

Training
--------
Optimizer    : Adam  (lr = 6.83e-3, weight_decay = 1.38e-3)
...
# Hyperparameters
EPOCHS       = 500
BATCH_SIZE   = 32        # optimized: smaller batch → better gradient signal
LR           = 6.83e-3   # optimized: higher than default 1e-3
WEIGHT_DECAY = 1.38e-3   # optimized: L2 regularization
DROPOUT      = (0.311, 0.184)  # optimized: per hidden layer
N_SPLITS     = 10
N_REPEATS    = 10
...
class DqBPredictor(nn.Module):
    ...
    Architecture: 15 → 64 → 32 → 1  (pyramidal, Optuna-optimized hyperparams)
    ...
    def __init__(self, input_dim: int = 15):
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm1d(64),  nn.ReLU(), nn.Dropout(d[0]),

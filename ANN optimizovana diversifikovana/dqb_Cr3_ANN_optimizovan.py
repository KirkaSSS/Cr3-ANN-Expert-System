...
Architecture
------------
Input (15) → Linear(128) → BN → ReLU → Dropout(0.379)
           → Linear(32)  → BN → ReLU → Dropout(0.171)
           → Linear(1)

Pyramidal architecture (128→32) determined by Bayesian hyperparameter
optimization (Optuna TPE, 50 trials, 10-fold CV) on diversified
To_predict set spanning all crystal field regimes (WCF/Edge/Tier1/SCF).

Training
--------
Optimizer    : Adam  (lr = 7.97e-3, weight_decay = 3.54e-4)
...
# Hyperparameters
EPOCHS       = 500
BATCH_SIZE   = 64        # optimized: stable gradient signal
LR           = 7.97e-3   # optimized: Trial 43 best
WEIGHT_DECAY = 3.54e-4   # optimized: lower L2 than previous search
DROPOUT      = (0.379, 0.171)  # optimized: per hidden layer
N_SPLITS     = 10
N_REPEATS    = 10
...
class DqBPredictor(nn.Module):
    ...
    Architecture: 15 → 128 → 32 → 1  (pyramidal, Optuna-optimized)
    BatchNorm + ReLU + Dropout after each hidden layer.
    He (Kaiming Normal) weight initialization.

    Determined by Bayesian optimization on diversified To_predict set
    spanning WCF, Edge, Tier 1, and SCF crystal field regimes.
    Best CV R² = 0.4318 (Trial 43, 50 trials total).
    """
    ...
    def __init__(self, input_dim: int = 15):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 32),
            nn.BatchNorm1d(32),  nn.ReLU(), nn.Dropout(d[1]),
